...
exports.patchOfferTemplateXml = patchOfferTemplateXml;
const T = {
...
function patchOfferTemplateXml(xml) {
    let s = xml;
    s = s.replace('«______________» (далее Произведение)', `«${T.workTitle}» (далее Произведение)`);
    s = s.replace(/<w:p w14:paraId="23EB6296"[\s\S]*?<w:t(?: xml:space="preserve")?>\u00a0<\/w:t>/, (m) => m.replace('>\u00a0</w:t>', `>${T.offerDateRu}</w:t>`));
    ...
    if (!distributorPara.test(s)) {
        throw new Error('Шаблон оффера: не найден абзац дистрибьютора (para 397BC326). Обновите DOCX.');
    }
    s = s.replace(distributorPara, distributorReplacement);
    s = s.replace('<w:t xml:space="preserve"> </w:t></w:r></w:p><w:p w14:paraId="6D43585E"', `<w:t xml:space="preserve">${T.contentTitle}</w:t></w:r></w:p><w:p w14:paraId="6D43585E"`);
    s = s.replace('<w:t xml:space="preserve"> </w:t></w:r></w:p><w:p w14:paraId="61D4C771"', `<w:t xml:space="preserve">${T.productionYear}</w:t></w:r></w:p><w:p w14:paraId="61D4C771"`);
    const valueP = (tag) => `<w:p w14:paraId="77FFFFFF" w14:textId="77777777" w:rsidR="001756BE" w:rsidRDefault="001756BE" w:rsidP="00D77535"><w:pPr><w:jc w:val="both"/><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/></w:rPr></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/></w:rPr><w:t>${tag}</w:t></w:r></w:p>`;
    s = s.replace('</w:t></w:r></w:p><w:p w14:paraId="4B7DB849"', `</w:t></w:r></w:p>${valueP(T.contentFormat)}<w:p w14:paraId="4B7DB849"`);
    s = s.replace('</w:t></w:r></w:p><w:p w14:paraId="410C6DF6"', `</w:t></w:r></w:p>${valueP(T.genre)}<w:p w14:paraId="410C6DF6"`);
    s = s.replace('</w:t></w:r></w:p><w:p w14:paraId="4D06C4BD"', `</w:t></w:r></w:p>${valueP(T.seriesCount)}<w:p w14:paraId="4D06C4BD"`);
    s = s.replace('</w:t></w:r></w:p><w:p w14:paraId="4BA1F074"', `</w:t></w:r></w:p>${valueP(T.runtime)}<w:p w14:paraId="4BA1F074"`);
    s = s.replace('</w:t></w:r></w:p><w:p w14:paraId="21707752"', `</w:t></w:r></w:p>${valueP(T.theatricalRelease)}<w:p w14:paraId="21707752"`);
    s = s.replace('</w:t></w:r></w:p><w:p w14:paraId="1E9277F8"', `</w:t></w:r></w:p>${valueP(T.rightsHolder)}<w:p w14:paraId="1E9277F8"`);
    s = s.replace(/(<w:p w14:paraId="1E9277F8"[\s\S]*?<\/w:p>)(<\/w:tc>)/, `$1${valueP(T.contentLanguage)}$2`);
    s = s.replace(/Исключительные права\s*\/\s*Не исключительные права/g, T.rightsParagraph);
    ...
    s = s.replace('- промо материалы</w:t></w:r></w:p></w:tc>', `- промо материалы</w:t></w:r></w:p>${valueP(T.materialsNote)}</w:tc>`);
    ...
    s = s.replace(/<w:p w14:paraId="482B0D44"[\s\S]*?<w:p w14:paraId="33B37C32"[\s\S]*?<\/w:p>/, additionalPara);
    ...
    s = s.replace(/<w:p w14:paraId="60AB730B"[\s\S]*?<\/w:p>/, rightsOpeningProcedurePara);
    ...
    s = s.replace(/<w:p w14:paraId="0FF68C2E"[\s\S]*?<\/w:p>/, remunerationPara);
    ...
    s = s.replace('** Предложение действительно 7 дней с момента отправки. ', `** Предложение действительно ${T.offerValidityDays} дней с момента отправки. `);
    s = s.replace('<w:t>________________________</w:t>', `<w:t>${T.signatoryLine}</w:t>`);
    return s;
}
//# sourceMappingURL=offer-template.patch-xml.js.map